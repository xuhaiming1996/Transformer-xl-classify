...
class MyAttention(nn.Module):
    ...
    def forward(self, inputs, time_major=True, return_alphas=False):
        ...
        alphas = exps /(torch.sum(input=exps,dim=1, keepdim=True)+0.00001)
...
class MemTransformerLM(nn.Module):
    def __init__(self, n_token, n_layer, n_head, d_model, d_head, d_inner,
        ...
        # 创建 attention
        self.attention_layer_low  = MyAttention(self.d_model, self.d_model//2)
        self.attention_layer_high = MyAttention(self.d_model, self.d_model // 2)

        self.fc = nn.Sequential(
            nn.Linear(self.d_model,self.d_model),
            nn.BatchNorm1d(self.d_model),
            nn.ReLU(inplace=True),
            nn.Linear(self.d_model, 19)
        )
    ...
    def _forward(self, dec_inp, mems=None):
        ...
        if self.same_length:
            ...
            if mask_len > 0:
                ...
                mask_shift_len = qlen
            dec_attn_mask = (torch.triu(all_ones, mlen)
            ...
            #
            dec_attn_mask = torch.triu(
                word_emb.new_ones(qlen, klen), diagonal=mlen).byte()[:,:,None]
    ...
    '''
     data, labels,bsz_len, args.tgt_len, device = 'cpu', ext_len = args.ext_len, *mems)
    '''
    def forward(self, data, labels,  *mems, tgt_len=None, device=None, ext_len=None):
        ...
        train_iter_one_batch = LMOrderedIterator(data, bsz_len, tgt_len, device=device, ext_len=ext_len)

        memory_low = []
        for index, (data, seq_len) in enumerate(train_iter_one_batch.get_fixlen_iter()):
            # tgt_len = target.size(0)
            # print(data)
            hidden, mems = self._forward(data, mems=mems)
            # print(mems)
            memory_low.append(self.attention_layer_low(hidden.permute(1,0,2)))
        #
        # print(memory_low)
        shapes=memory_low[0].size()

        # print(len(memory_low))
        # print(shapes)
        memory_high = torch.cat(memory_low, 0)
        memory_high = torch.reshape(memory_high, (-1, shapes[0], shapes[1]))
        # print(memory_high.size())
        res=self.attention_layer_high(memory_high.permute(1,0,2))
        #########下面添加映射层 计算损失函数##############
        # print(res)
        logits=self.fc(res)
        # print(logits.size())
