...
# 这是词典类
class Vocab(object):
    def __init__(self, alinlen=3000,special=[], min_freq=10, max_size=None, lower_case=True,
    ...
    def _build_from_file(self, vocab_file):
        ...
        self.s_idx   = self.sym2idx['<s>']
        self.unk_idx = self.sym2idx['<unk>']
        setattr(self, '{}_idx'.format("</s>".strip('<>')), self.sym2idx['</s>'])
    ...
    def encode_file_only_for_lables(self, path, verbose=True):

        if verbose:
            print('正在编码标签 {} ...'.format(path))
        assert os.path.exists(path)
        encoded = []
        with open(path, 'r', encoding='utf-8') as f:
            for idx, line in enumerate(f):
    ...
    def encode_file(self, path, verbose=False):
        '''
                           ordered=True
        :param path:
        :param ordered:
        ...
        encoded = torch.reshape(torch.cat(encoded,0), (-1, 3000))
        print(encoded.size())
